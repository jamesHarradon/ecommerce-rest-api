...
class CustomerModel {
    ...
    async checkExistingUserName(username) {
        try {
            const data = await pool.query('SELECT * FROM customers WHERE user_name = $1', [username]);
    ...
    async getCustomerData(custid) {
        try {
            const data = await pool.query('SELECT customers.id as customer_id, contacts.id as contact_id, payment_id, first_name, last_name, user_name, address_line1, address_line2, town, city, county, post_code, phone, email FROM customers JOIN contacts ON customers.contact_id = contacts.id WHERE customers.id = $1', [custid]);
            return data.rows?.length ? data.rows[0] : null;
        } catch (err) {
            throw new Error(err);
        }
